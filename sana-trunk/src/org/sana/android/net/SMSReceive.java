...
public class SMSReceive extends BroadcastReceiver {
	...
	private void processNotificationMessage(Context context, MDSNotification notificationHeader, String message) {
		...
		Cursor c = context.getContentResolver().query(Notifications.CONTENT_URI, 
				new String[] { Notifications._ID, Notifications.PATIENT_ID, Notifications.PROCEDURE_ID, Notifications.MESSAGE }, 
		...
		if (notificationHeader.p != null) {
			...
			cv.put(Notifications.PATIENT_ID, notificationHeader.p);
		}
		if (notificationHeader.c != null) {
			cv.put(Notifications.PROCEDURE_ID, notificationHeader.c);
		}
		Uri notificationUri;
		...
		if (c.moveToFirst()) {
			...
			int notificationId = c.getInt(c.getColumnIndex(Notifications._ID));
			String storedMessage = c.getString(c.getColumnIndexOrThrow(Notifications.MESSAGE));
			
			NotificationMessage m = g.fromJson(storedMessage, NotificationMessage.class);
			
			if (patientId == null) {
				patientId = c.getString(c.getColumnIndex(Notifications.PATIENT_ID));
			}
			...
			if (notificationHeader.d != null) {
				...
				if (matcher.matches()) {
					...
					if (m.totalMessages == m.receivedMessages) { 
						...
						cv.put(Notifications.FULL_MESSAGE, fullMessage);
						cv.put(Notifications.DOWNLOADED, 1);
					}
				}
			} else {
			...
			cv.put(Notifications.MESSAGE, storedMessage);
			
			notificationUri = ContentUris.withAppendedId(Notifications.CONTENT_URI, notificationId);
			...
			if (notificationHeader.d != null) {
				...
				cv.put(Notifications.FULL_MESSAGE, message);
				cv.put(Notifications.DOWNLOADED, 1);
				complete = true;
			}
			...
			cv.put(Notifications.MESSAGE, storedMessage);
			cv.put(Notifications.NOTIFICATION_GUID, notificationHeader.n);
			notificationUri = context.getContentResolver().insert(Notifications.CONTENT_URI, cv);
		}
