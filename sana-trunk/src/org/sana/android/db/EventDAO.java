...
public class EventDAO {
			...
			String encounterRef, String patientRef, String userRef) 
	{
		...
		cv.put(Events.EVENT_TYPE, type.toString());
		cv.put(Events.EVENT_VALUE, value);
		
		if (encounterRef != null)
			cv.put(Events.ENCOUNTER_REFERENCE, encounterRef);
		if (patientRef != null)
			cv.put(Events.PATIENT_REFERENCE, patientRef);
		if (userRef != null)
			cv.put(Events.USER_REFERENCE, userRef);
