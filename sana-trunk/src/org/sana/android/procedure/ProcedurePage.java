...
public class ProcedurePage {
	...
	public ProcedurePage(String id, List<ProcedureElement> elements) {
	...
	public ProcedurePage(List<ProcedureElement> elements, Criteria criteria) {
		this("",elements,criteria);
	}
	
	public ProcedurePage(String id, List<ProcedureElement> elements, Criteria criteria) {
		this.id=id;
	...
	private String[] specialElementsNew = {"patientIdNew", 
			"patientFirstNameNew", "patientLastNameNew", "patientBirthdateDayNew", "patientBirthdateMonthNew", "patientBirthdateYearNew", "patientGenderNew"};
	...
	public boolean hasSpecialElementNew() {
		for (int i=0; i<elements.size(); i++) {
			for (int j=0; j<specialElementsNew.length; j++) {
				if (elements.get(i).id.equals(specialElementsNew[j])) {
					return true;
				}
			}
		}
	...
	/**
	 * Returns a list of all the special elements on the page
	 */
	...
	public List<ProcedureElement> getSpecialElementsNew() {
		List<ProcedureElement> els = new ArrayList<ProcedureElement>();
		for (ProcedureElement el : elements) {
			if (isSpecialElementNew(el))
				els.add(el);
		}
		return els;
	}
	...
	public boolean isSpecialElementNew(ProcedureElement e) {
		for (int i=0; i<specialElementsNew.length; i++) {
			if (e.id.equals(specialElementsNew[i])) {
				return true;
			}
		}
		return false;
	...
	public boolean shouldDisplay() {
		Log.i(TAG, "shouldDisplay() called");
